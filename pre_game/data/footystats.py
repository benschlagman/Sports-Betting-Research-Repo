...
class GenerateDataFrame():
    ...
    def get_filtered_leagues(self, league_name, country, years):
        ...
        filtered_leagues = leagues[(leagues['country'] == country) & (
            leagues['league_name'] == league_name)]
        ...
        all_leagues_data = all_leagues_data[all_leagues_data['year'].isin(
            years)]
    ...
    def load(self, is_save=False) -> pd.DataFrame:
        df = self.get_footystats_matches(
            self.league_name, self.country, self.years)
        if is_save:
            df.to_csv('raw_footystats.csv', index=False)

        return FootyStatsCleaner(df).run()
